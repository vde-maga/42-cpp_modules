...
void test_constructors_and_size()
{
	...
	if (empty.size() == 0)
		TestUI::printPass("Default constructor creates empty array");
	else
		TestUI::printFail("Default constructor failed");

	if (sized.size() == 5)
		TestUI::printPass("Parameterized constructor creates sized array");
	else
		TestUI::printFail("Parameterized constructor failed");
}
...
void test_operator_brackets_and_exceptions()
{
	...
	if (pass)
		TestUI::printPass("Operator[] works and throws on out of bounds");
	else
		TestUI::printFail("Operator[] or exception handling failed");
}
...
void test_const_correctness()
{
	...
	try
	{
		if (const_ref[0] != "Hello" || const_ref[1] != "World")
			pass = false;
	...
	if (pass)
		TestUI::printPass("Const operator[] allows read-only access");
	else
		TestUI::printFail("Const operator[] failed");
}
